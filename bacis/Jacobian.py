...
class Elem4:
    def __init__(self):
        ...
        self.cp = fem.Grid.global_data.cp
        self.ro = fem.Grid.global_data.ro
    ...
    def calculateH(self,pc):
        for i in range(0,4):
            for j in range(0,4):
                self.H[i][j] = (self.finalNx[i][j] + self.finalNy[i][j]) * self.k * self.detJ * self.ksiWeight[pc] * self.etaWeight[pc]
    ...
    def calculateC(self, pc):
        for i in range(0,4):
            for j in range(0,4):
                self.C[i][j] = (self.N[pc][i] * self.N[pc][j]) * self.detJ * self.cp * self.ro * self.etaWeight[pc] * self.ksiWeight[pc]
